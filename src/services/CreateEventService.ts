...
class CreateEventService {
    public async execute({
        ...
        if (
            isBefore(hourStart, new Date()) ||
            isBefore(hourEnd, new Date()) ||
            isBefore(new Date(end_date), new Date(start_date))
        ) {
        ...
        const casting_start_date = new Date(start_date);
        const casting_end_date = new Date(end_date);

        const event = eventsRepository.create({
            user_id,
            description,
            start_date: casting_start_date,
            end_date: casting_end_date,
        });
